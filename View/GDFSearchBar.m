...
@interface GDFSearchBar() <UITextFieldDelegate, UIGestureRecognizerDelegate>
/**
...
@property (nonatomic, assign) GDFSearchBarState state;
@end

static CGFloat const kGDFSearchBarInset = 11.0;
static CGFloat const kGDFSearchBarImageSize = 22.0;
static NSTimeInterval const kGDFSearchBarAnimationStepDuration = 0.25;
@implementation GDFSearchBar
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:CGRectMake(CGRectGetMinX(frame), CGRectGetMinY(frame), CGRectGetWidth(frame), 34.0)]))
    {
        ...
        self.searchField = [[UITextField alloc] initWithFrame:CGRectMake(kGDFSearchBarInset, 3.0, CGRectGetWidth(self.bounds) - (2 * kGDFSearchBarInset) - kGDFSearchBarImageSize, CGRectGetHeight(self.bounds) - 6.0)];
        ...
        UIView *searchImageViewOnContainerView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kGDFSearchBarInset - kGDFSearchBarImageSize, (CGRectGetHeight(self.bounds) - kGDFSearchBarImageSize) / 2, kGDFSearchBarImageSize, kGDFSearchBarImageSize)];
        ...
        self.searchImageViewOff = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kGDFSearchBarInset - kGDFSearchBarImageSize, (CGRectGetHeight(self.bounds) - kGDFSearchBarImageSize) / 2, kGDFSearchBarImageSize, kGDFSearchBarImageSize)];
        ...
        UIView *tapableView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - (2 * kGDFSearchBarInset) - kGDFSearchBarImageSize, 0.0, (2 * kGDFSearchBarInset) + kGDFSearchBarImageSize, CGRectGetHeight(self.bounds))];
...
- (void)changeStateIfPossible:(UITapGestureRecognizer *)gestureRecognizer
{
    switch (self.state)
    {
        case GDFSearchBarStateNormal:
        {
            [self showSearchBar:gestureRecognizer];
        }
            break;
            
        case GDFSearchBarStateSearchBarVisible:
        {
            [self hideSearchBar:gestureRecognizer];
        }
            break;
            
        case GDFSearchBarStateSearchBarHasContent:
        {
        ...
        case GDFSearchBarStateTransitioning:
        {
            // Do nothing.
        }
            break;
...
{
    if (self.state == GDFSearchBarStateNormal)
    {
        if ([self.delegate respondsToSelector:@selector(searchBar:willStartTransitioningToState:)])
        {
            [self.delegate searchBar:self willStartTransitioningToState:GDFSearchBarStateSearchBarVisible];
        }
        
        self.state = GDFSearchBarStateTransitioning;
        
        self.searchField.text = nil;
        
        [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration animations:^{
            ...
            [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration * 2 animations:^{
                ...
                self.state = GDFSearchBarStateSearchBarVisible;
                
                if ([self.delegate respondsToSelector:@selector(searchBar:didEndTransitioningFromState:)])
                {
                    [self.delegate searchBar:self didEndTransitioningFromState:GDFSearchBarStateNormal];
                }
            }];
        }];
    }
...
{
    if (self.state == GDFSearchBarStateSearchBarVisible || self.state == GDFSearchBarStateSearchBarHasContent)
    {
        [self.window endEditing:YES];
        
        if ([self.delegate respondsToSelector:@selector(searchBar:willStartTransitioningToState:)])
        {
            [self.delegate searchBar:self willStartTransitioningToState:GDFSearchBarStateNormal];
        }
        ...
        [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration animations:^{
            ...
            [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration animations:^{
                ...
                self.state = GDFSearchBarStateNormal;
                
                if ([self.delegate respondsToSelector:@selector(searchBar:didEndTransitioningFromState:)])
                {
                    [self.delegate searchBar:self didEndTransitioningFromState:GDFSearchBarStateSearchBarVisible];
                }
            }];
        }];
    }
...
- (void)dismissKeyboard:(UITapGestureRecognizer *)gestureRecognizer
{
    if ([self.searchField isFirstResponder])
    {
        ...
        if (self.state == GDFSearchBarStateSearchBarVisible && self.searchField.text.length == 0)
        {
...
- (void)textDidChange:(NSNotification *)notification
{
    ...
    if (hasText)
    {
        if (self.state == GDFSearchBarStateSearchBarVisible)
        {
            self.state = GDFSearchBarStateTransitioning;
            ...
            [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration animations:^{
                ...
                [UIView animateWithDuration:kGDFSearchBarAnimationStepDuration animations:^{
                    ...
                    self.state = GDFSearchBarStateSearchBarHasContent;
    ...
    {
        if (self.state == GDFSearchBarStateSearchBarHasContent)
        {
            self.state = GDFSearchBarStateTransitioning;
